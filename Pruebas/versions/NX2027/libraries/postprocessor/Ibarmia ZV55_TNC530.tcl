########################## TCL Event Handlers ##########################
#
#  Ibarmia ZV55_TNC530.tcl - 5_axis_dual_head
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Heads.
#
#  Created by bugalde @ Tuesday, January 26 2021, 09:49:59 +0100
...
# 08-09-2016 lili - Fix ZYX rotation angle calculation issue in DPP_GE_CALCULATE_COOR_ROT_ANGLE. zero resolution was too high.
  ...
  set mom_sys_alt_unit_post_name                "Ibarmia ZV55_TNC530__IN.pui"
...
#=============================================================
  ...
  # Accumulated time should be in minutes.
   ...
   PB_CMD_salida_lbl101
   PB_CMD_reset_output_mode
   PB_CMD_reset_final_values
   PB_CMD_reset_variables
   PB_CMD_save_axis_leader
...
#=============================================================
   ...
   PB_CMD_uplevel_MOM_Refrigerante_EUSKO
...
global mom_out_angle_pos mom_alt_pos mom_tool_axis
...
if { $mom_post_in_simulation == "CSE"} {
#MOM_do_template plane_spatial_turn
return 1
} else {
 return 1
}
    ...
    if { [info exists mom_siemens_feed_definition] && $mom_siemens_feed_definition == "ON" } {
        ...
        if {![string compare "Hole Making" $mom_operation_type] || ![string compare "Point to Point" $mom_operation_type] || \
          ![string compare "Drilling" $mom_operation_type] } {
             PB_CMD_get_feed_value
             #MOM_output_literal "FN 0:Q91=$mom_user_feed_value(engage) ;(Avance de entrada)"
             #MOM_output_literal "FN 0:Q92=$mom_user_feed_value(first_cut) ;(Avance de primer corte)"
             MOM_output_literal "FN 0:Q92=$mom_user_feed_value(cut) ;(Avance de trabajo)"
             #MOM_output_literal "FN 0:Q94=$mom_user_feed_value(traversal) ;(Avance de transversal)"
             #MOM_output_literal "FN 0:Q95=$mom_user_feed_value(retract) ;(Avance de salida)"
        } else {
            PB_CMD_get_feed_value
           MOM_output_literal "FN 0:Q91=$mom_user_feed_value(engage) ;(Avance de entrada)"
             MOM_output_literal "FN 0:Q92=$mom_user_feed_value(first_cut) ;(Avance de primer corte)"
             MOM_output_literal "FN 0:Q93=$mom_user_feed_value(cut) ;(Avance de trabajo)"
             MOM_output_literal "FN 0:Q94=$mom_user_feed_value(traversal) ;(Avance de transversal)"
             MOM_output_literal "FN 0:Q95=$mom_user_feed_value(retract) ;(Avance de salida)"
			 MOM_output_literal "FN 0:Q96=$mom_user_feed_value(approach) ;(Avance de aproximacion)"
...
# string value.
  ...
  if {[info exists mom_ude_feed_definition] && $mom_ude_feed_definition == "ON"} {
     ...
     switch $motion_type {
        "cut" -
        "firstcut" -
        "stepover" { set mom_user_feed "Q92"}
        "engage" { set mom_user_feed "Q91"}
        "retract" { set mom_user_feed "Q92" }
        "approach" { set mom_user_feed "Q96" }
        default {
...
global flag_param_lab
global flag_label_acabado
global mom_label_mill mom_label_finish
...
#=============================================================
MOM_output_literal "; ==========================="
MOM_output_literal "; ==== Ibarmia ZV55 ===="
MOM_output_literal "; ==========================="
...
#MOM_output_literal "mom_out_angle_pos(0)    $mom_out_angle_pos(0)"
#MOM_output_literal "mom_out_angle_pos(1)    $mom_out_angle_pos(1)"
...
MOM_output_literal "L Z1800 FMAX M91"
...
# 06-18-2013 Jason - Initial version
  ...
  global mom_cambio_plano tipo_op
...
#Cancel Plane Special
  if { $tipo_op == "3mas2" } {
     ...
     #MOM_force once G_motion G Z M
     #MOM_do_template return_home_z
     #MOM_force once G_motion G X Y M
     #MOM_do_template return_home_xy
...
if { $mom_tool_axis(1) > "0.999" } {
    ...
    set flag_comp_ciclo7    "1"

    set A   [expr -1*$mom_alt_pos(4)]
    set B   $mom_alt_pos(3)
    set A   [format "%3.2f" $A]
    set B   [format "%3.2f" $B]
     MOM_output_literal "; Compensacion cabezal"
     MOM_output_literal "CYCL DEF 7.0"
     MOM_output_literal "CYCL DEF 7.1 IX+148.21"
     MOM_output_literal "CYCL DEF 7.2 IZ+148.21"
    ...
    set B   [format "%3.2f" $B]
     MOM_output_literal "; Compensacion cabezal"
     MOM_output_literal "CYCL DEF 7.0"
     MOM_output_literal "CYCL DEF 7.1 IZ+298.43"
...
proc PB_CMD_avisos { } {
#=============================================================
global mom_tool_pitch mom_current_motion mom_tool_name
