########################## TCL Event Handlers ##########################
#
#  Soraluce_FLP8000.tcl - 5_axis_dual_head
#
#    This is a 5-Axis Milling Machine With
#     Dual Rotary Heads.
#
#  Created by CRuiz @ Tuesday, November 18 2025, 14:55:02 +0100
...
# 08-09-2016 lili - Fix ZYX rotation angle calculation issue in DPP_GE_CALCULATE_COOR_ROT_ANGLE. zero resolution was too high.
  ...
  set mom_sys_alt_unit_post_name                "Soraluce_FLP8000__IN.pui"
...
global mom_pos_alt mom_pos mom_out_angle_pos mom_alt_pos ats_mom_out_pos
    ...
    if { [info exists mom_siemens_feed_definition] && $mom_siemens_feed_definition == "ON" } {
        if {![string compare "Hole Making" $mom_operation_type] || ![string compare "Point to Point" $mom_operation_type] || \
          ![string compare "Drilling" $mom_operation_type] } {
             PB_CMD_get_feed_value
             #MOM_output_literal "FN 0:Q1601=$mom_user_feed_value(engage) ;(Avance de entrada)"
             #MOM_output_literal "FN 0:Q1602=$mom_user_feed_value(first_cut) ;(Avance de primer corte)"
             MOM_output_literal "FN 0:Q1602=$mom_user_feed_value(cut) ;(Avance de trabajo)"
             #MOM_output_literal "FN 0:Q1604=$mom_user_feed_value(traversal) ;(Avance de transversal)"
             #MOM_output_literal "FN 0:Q1605=$mom_user_feed_value(retract) ;(Avance de salida)"
        } else {
             PB_CMD_get_feed_value
             MOM_output_literal "FN 0:Q1601=$mom_user_feed_value(engage) ;(Avance de entrada)"
             MOM_output_literal "FN 0:Q1602=$mom_user_feed_value(first_cut) ;(Avance de primer corte)"
             MOM_output_literal "FN 0:Q1603=$mom_user_feed_value(cut) ;(Avance de trabajo)"
             MOM_output_literal "FN 0:Q1604=$mom_user_feed_value(traversal) ;(Avance de transversal)"
             MOM_output_literal "FN 0:Q1605=$mom_user_feed_value(retract) ;(Avance de salida)"
             MOM_output_literal "FN 0:Q1606=$mom_user_feed_value(approach) ;(Avance de aproximacion)"
        }
    }
...
# string value.
  ...
  if {[info exists mom_ude_feed_definition] && $mom_ude_feed_definition == "ON"} {
     ...
     switch $motion_type {
        "cut" -
        "firstcut" -
        "stepover" { set mom_user_feed "Q1602"}
        "engage" { set mom_user_feed "Q1601"}
        "retract" { set mom_user_feed "Q1602" }
        "approach" { set mom_user_feed "Q1606" }
        default {
...

     switch $motion_type {
        "cut" -
        "firstcut" -
        "stepover" { set mom_user_feed "Q1603"}
        "engage" { set mom_user_feed "Q1603"}
        "retract" { set mom_user_feed "Q1604"}
        "rapid" {
...
#=============================================================
MOM_output_literal "; ==========================="
MOM_output_literal "; ==== SORALUCE FLP 8000 ===="
MOM_output_literal "; ==========================="
...
#MOM_output_literal "; Out_0 = $mom_out_angle_pos(0) Out_0 = $mom_out_angle_pos(1)"
set A "+Q120"
set B "+Q121"
